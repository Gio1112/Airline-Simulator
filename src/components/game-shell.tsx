...
export function GameShell() {
  ...
  const [notice, setNotice] = useState("Prototype economy is running locally. Supabase schema is included for the next milestone.");
    ...
    <main className="mx-auto min-h-screen max-w-7xl px-4 py-5 sm:px-6 lg:px-8">
      ...
      <nav className="mb-5 flex gap-1 rounded-xl border border-[var(--border)] bg-[var(--panel)] p-1">
        {(["overview", "fleet", "routes"] as const).map((item) => (
          <button key={item} onClick={() => setTab(item)} className={`rounded-lg px-4 py-2 text-sm capitalize ${tab === item ? "bg-slate-700 text-white" : "text-[var(--muted)] hover:text-white"}`}>{item}</button>
        ))}
      </nav>

      {tab === "overview" && <Overview airline={airline} results={results} />}
      {tab === "fleet" && <Fleet airline={airline} setAirline={setAirline} setNotice={setNotice} />}
      {tab === "routes" && <Routes airline={airline} setAirline={setAirline} setNotice={setNotice} results={results} />}
    </main>
  );
}
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-5xl items-center px-4 py-12">
      <div className="grid w-full gap-10 lg:grid-cols-[1.2fr_0.8fr] lg:items-center">
        <section>
          <div className="mb-4 text-xs font-semibold uppercase tracking-[0.28em] text-sky-300">Airline Simulator · v0.1</div>
          ...
          <p className="mt-5 max-w-xl text-lg leading-8 text-[var(--muted)]">Start with $25M, lease aircraft, build routes, balance capacity against demand, and grow a profitable network.</p>
        </section>
        <form className="rounded-2xl border border-[var(--border)] bg-[var(--panel)] p-5" onSubmit={(event) => {
          ...
          <label className="mt-4 block text-sm text-[var(--muted)]">Starting hub<select value={hub} onChange={(e) => setHub(e.target.value)} className="mt-2 w-full rounded-lg border border-[var(--border)] bg-[var(--background)] px-3 py-2.5 text-white outline-none focus:border-sky-300">{airports.map((airport) => <option key={airport.iata} value={airport.iata}>{airport.iata} — {airport.city}</option>)}</select></label>
...
function Overview({ airline, results }: { airline: Airline; results: RouteResult[] }) {
  ...
  return <div className="space-y-5">
    <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{kpis.map(([label, value, helper]) => <div key={label} className="rounded-xl border border-[var(--border)] bg-[var(--panel)] p-4"><div className="text-xs uppercase tracking-wider text-[var(--muted)]">{label}</div><div className="mt-2 text-2xl font-semibold">{value}</div><div className="mt-1 text-xs text-[var(--muted)]">{helper}</div></div>)}</section>
    <section className="rounded-xl border border-[var(--border)] bg-[var(--panel)] p-5">
      <div className="flex items-center justify-between"><div><h2 className="font-semibold">Network</h2><p className="text-sm text-[var(--muted)]">Current hub-and-spoke structure</p></div><div className="rounded-full border border-[var(--border)] px-3 py-1 text-xs text-[var(--muted)]">Hub {airline.hub}</div></div>
      <div className="mt-6 grid min-h-64 place-items-center rounded-lg border border-dashed border-[var(--border)] bg-[var(--background)] p-5">
        {airline.routes.length === 0 ? <div className="text-center"><div className="text-lg font-medium">No routes yet</div><div className="mt-1 text-sm text-[var(--muted)]">Lease an aircraft, then open your first route.</div></div> : <div className="grid w-full gap-2 sm:grid-cols-2 lg:grid-cols-3">{airline.routes.map((route) => <div key={route.id} className="rounded-lg border border-[var(--border)] bg-[var(--panel-2)] p-3"><div className="font-mono text-lg">{route.origin} <span className="text-sky-300">→</span> {route.destination}</div><div className="mt-1 text-xs text-[var(--muted)]">{distanceKm(route.origin, route.destination).toLocaleString()} km · {route.weeklyFrequency}× weekly · {money(route.economyFare)}</div></div>)}</div>}
      </div>
    </section>
  </div>;
}

function Fleet({ airline, setAirline, setNotice }: { airline: Airline; setAirline: (updater: Airline | ((current: Airline | null) => Airline | null)) => void; setNotice: (notice: string) => void }) {
...
function Routes({ airline, setAirline, setNotice, results }: { airline: Airline; setAirline: (updater: Airline | ((current: Airline | null) => Airline | null)) => void; setNotice: (notice: string) => void; results: RouteResult[] }) {
  const freeAircraft = airline.fleet.filter((aircraft) => !airline.routes.some((route) => route.aircraftId === aircraft.id));
  const [destination, setDestination] = useState(airports.find((a) => a.iata !== airline.hub)?.iata ?? "LAX");
